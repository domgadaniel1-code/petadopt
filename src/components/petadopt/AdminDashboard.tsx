...
import {
  ...
  Plus, Pencil, Trash2, Search, Filter, Lock,
  UserCheck, AlertCircle, Settings,
  Dog, Cat, MapPin, Calendar, DollarSign, Activity, Star, Heart
} from 'lucide-react'
...
export default function AdminDashboard() {
  const {
    ...
    adminUsers, addUser, updateUser, deleteUser, banUser, unbanUser,
  } = usePetAdoptStore()
  ...
  // ALL useState hooks MUST be before any conditional return (Rules of Hooks)
      ...
      <div className="min-h-screen pt-16 flex items-center justify-center bg-muted/20">
        <Card className="max-w-md w-full mx-4 border-0 shadow-xl">
          <CardContent className="p-8 text-center">
            ...
            <p className="text-muted-foreground mb-6">Cet espace est reserve aux administrateurs de la plateforme PetAdopt.</p>
  ...
  const disputes = [
    { id: 'd1', userName: 'Marie D.', description: 'Animal malade a la reception', status: 'OPEN', date: '2026-05-28' },
    { id: 'd2', userName: 'Thomas L.', description: 'Remboursement non recu', status: 'IN_PROGRESS', date: '2026-05-25' },
              ...
              <CardContent className="p-4">
                ...
                <nav className="space-y-1">
                  ...
                  <button onClick={() => { logout(); navigate('home') }} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-destructive hover:bg-destructive/10 transition-all">
                    <ArrowLeft className="h-4 w-4" /> Deconnexion
                  </button>
                </nav>
              </CardContent>
            </Card>
          ...
          <div className="flex-1 min-w-0">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  {[
                    ...
                    { label: 'Revenus', value: `${totalRevenue.toLocaleString('fr-FR')} EUR`, sub: `${pendingRevenue} EUR en attente`, icon: TrendingUp, color: 'text-green-500', bg: 'bg-green-500/10' },
                ...
                <Card className="border-0 shadow-sm">
                  <CardHeader><CardTitle className="text-lg">Activite de la plateforme</CardTitle></CardHeader>
                  <CardContent>
                    <div className="flex items-end gap-3 h-48">
                      {['Jan', 'Fev', 'Mar', 'Avr', 'Mai', 'Jun', 'Jul', 'Aou'].map((m, i) => {
                  ...
                  <CardContent>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      {[
                        ...
                        { label: 'Gerer les animaux', icon: PawPrint, action: () => setActiveSection('animals'), color: 'bg-petorange/10 text-petorange' },
                        { label: 'Gerer les utilisateurs', icon: Users, action: () => setActiveSection('users'), color: 'bg-purple-500/10 text-purple-500' },
                      ].map((a, i) => (
                ...
                {/* Recent activity */}
                <Card className="border-0 shadow-sm">
                  <CardHeader><CardTitle className="text-lg">Activite recente</CardTitle></CardHeader>
                  <CardContent className="space-y-3">
                    {[
                      { text: 'Nouvel utilisateur inscrit : Marie Dupont', time: 'Il y a 2h', icon: Users },
                      { text: 'Nouvel animal ajoute : Rocky (Bulldog Francais)', time: 'Il y a 4h', icon: PawPrint },
                      { text: 'Paiement recu : 850 EUR pour Max', time: 'Il y a 6h', icon: CreditCard },
                      { text: 'Litige signale par Thomas L.', time: 'Il y a 1j', icon: AlertTriangle },
                    ].map((a, i) => (
                    ...
                    <div className="flex flex-wrap gap-3">
                      ...
                      <Select value={animalStatusFilter} onValueChange={setAnimalStatusFilter}>
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="ADOPTED">Adopte</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                  </CardContent>
                  ...
                  <Card className="border-amber-200 bg-amber-50 dark:bg-amber-950/20 dark:border-amber-900">
                    <CardContent className="p-4 flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-amber-600 dark:text-amber-500">Ces animaux ont ete ajoutes mais ne sont pas encore visibles publiquement.</p>
                      </div>
                    </CardContent>
                  </Card>
                )}
                  ...
                  {filteredAnimals.length === 0 && (
                    <Card className="border-0 shadow-sm">
                      <CardContent className="p-12 text-center">
                        ...
                        <p className="text-muted-foreground">Aucun animal trouve</p>
                      </CardContent>
                    </Card>
                  )}
                        ...
                        <div className="flex flex-col sm:flex-row gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-start justify-between gap-2 mb-2">
                              <div>
                                ...
                                <div className="flex items-center gap-2 mt-1 flex-wrap">
                                  ...
                                  <span className="text-sm text-muted-foreground">&#8226;</span>
                                  <span className="text-sm text-muted-foreground">{a.sex === 'MALE' ? 'Male' : 'Femelle'}</span>
                                </div>
                              </div>
                              <div className="flex items-center gap-2 shrink-0">
                                <Badge className={`${a.status === 'AVAILABLE' ? 'bg-green-500' : a.status === 'RESERVED' ? 'bg-amber-500' : 'bg-gray-500'} text-white border-0`}>
                                  {a.status === 'AVAILABLE' ? 'Disponible' : a.status === 'RESERVED' ? 'Reserve' : 'Adopte'}
                                </Badge>
                                {!a.isApproved && (
                                  <Badge className="bg-red-500 text-white border-0">
                                    <AlertCircle className="h-3 w-3 mr-1" /> Non approuve
                                  </Badge>
                                )}
                                {a.isApproved && (
                                  <Badge className="bg-green-500/10 text-green-600 border-green-200">
                                    <CheckCircle className="h-3 w-3 mr-1" /> Approuve
                                  </Badge>
                                )}
                              </div>
                            </div>
                            <div className="flex items-center gap-4 text-sm text-muted-foreground mb-2">
                              ...
                              <span className="font-bold text-foreground">{a.price.toLocaleString('fr-FR')} EUR</span>
                            </div>
                          ...
                          <div className="flex sm:flex-col gap-2 shrink-0 sm:ml-2">
                            ...
                            {!a.isApproved && (
                              <Button size="sm" className="flex-1 sm:flex-none sm:w-full text-xs gap-1.5 bg-green-500 hover:bg-green-600 text-white" onClick={() => { approveAnimal(a.id); showToast(`${a.name} a ete approuve`) }}>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {[
                    ...
                    { label: 'Eleveurs', value: users.filter(u => u.role === 'BREEDER').length, icon: Star, color: 'text-green-500', bg: 'bg-green-500/10' },
                  ].map((s, i) => (
                    ...
                    <div className="flex flex-wrap gap-3">
                      ...
                      <Select value={userRoleFilter} onValueChange={setUserRoleFilter}>
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="BREEDER">Eleveurs</SelectItem>
                      ...
                      <CardContent className="p-5">
                        <div className="flex flex-col sm:flex-row gap-4">
                            ...
                            <div className="flex items-start justify-between gap-2 mb-2">
                              ...
                              <div className="flex items-center gap-2 shrink-0">
                                <Badge variant="outline" className={`text-xs ${
                                  ...
                                  {u.role === 'ADMIN' ? 'Admin' : u.role === 'SHELTER' ? 'Refuge' : u.role === 'BREEDER' ? 'Eleveur' : 'Adoptant'}
                                </Badge>
                                ...
                                {u.isVerified ? (
                                  <Badge className="bg-green-500/10 text-green-600 border-green-200">
                                    <CheckCircle className="h-3 w-3 mr-1" /> Verifie
                                  </Badge>
                                ) : (
                                  <Badge className="bg-amber-500/10 text-amber-600 border-amber-200">
                                    <AlertCircle className="h-3 w-3 mr-1" /> Non verifie
                                  </Badge>
                                )}
                              </div>
                            </div>
                          ...
                          <div className="flex sm:flex-col gap-2 shrink-0 sm:ml-2">
                            ...
                            {/* MODIFIER */}
                            <Button size="sm" variant="outline" className="flex-1 sm:flex-none sm:w-full text-xs gap-1.5 text-petblue border-petblue/30 hover:bg-petblue/10" onClick={() => { setUserForm({name: u.name, email: u.email, role: u.role, phone: u.phone || '', city: u.city || '', country: u.country || '', bio: u.bio || ''}); setSelectedItem(u); setShowEditUser(true) }}>
                              <Pencil className="h-3.5 w-3.5" /> Modifier
                            </Button>
                            {/* BANNIR / DEBANNIR */}
                            <Button size="sm" variant="outline" className={`flex-1 sm:flex-none sm:w-full text-xs gap-1.5 ${u.isBanned ? 'text-green-600 border-green-300 hover:bg-green-50' : 'text-amber-600 border-amber-300 hover:bg-amber-50'}`} onClick={() => {
                              if (u.isBanned) {
                                unbanUser(u.id)
                                showToast(`${u.name} a ete debanni`)
                              } else {
                                banUser(u.id)
                                showToast(`${u.name} a ete banni`)
                              }
                            }}>
                              {u.isBanned ? <><UserCheck className="h-3.5 w-3.5" /> Debannir</> : <><Ban className="h-3.5 w-3.5" /> Bannir</>}
                            </Button>
                            {/* SUPPRIMER */}
                ...
                <div className="grid sm:grid-cols-3 gap-4">
                  <Card className="border-0 shadow-sm">
                    <CardContent className="p-5 flex items-center gap-4">
                      ...
                      <div><p className="text-2xl font-bold">{totalRevenue.toLocaleString('fr-FR')} EUR</p><p className="text-xs text-muted-foreground">Total encaisse</p></div>
                    </CardContent>
                  </Card>
                  <Card className="border-0 shadow-sm">
                    <CardContent className="p-5 flex items-center gap-4">
                      ...
                      <div><p className="text-2xl font-bold">{pendingRevenue.toLocaleString('fr-FR')} EUR</p><p className="text-xs text-muted-foreground">En attente</p></div>
                    </CardContent>
                  </Card>
                  <Card className="border-0 shadow-sm">
                    <CardContent className="p-5 flex items-center gap-4">
                      ...
                      <div><p className="text-2xl font-bold">0 EUR</p><p className="text-xs text-muted-foreground">Rembourse</p></div>
                    </CardContent>
                  </Card>
                </div>
                    ...
                    <table className="w-full">
                      <thead className="bg-muted/50">
                        <tr>
                          ...
                          <th className="text-left px-4 py-3 text-xs font-medium text-muted-foreground">Methode</th>
                          ...
                          <tr key={p.id} className="border-t">
                            ...
                            <td className="px-4 py-3">
                              <Badge className={`${p.status === 'COMPLETED' ? 'bg-green-500' : p.status === 'PENDING' ? 'bg-amber-500' : 'bg-red-500'} text-white border-0 text-xs`}>
                                {p.status === 'COMPLETED' ? 'Complete' : p.status === 'PENDING' ? 'En attente' : 'Echoue'}
                              </Badge>
                            </td>
                    ...
                    <Card key={d.id} className="border-0 shadow-sm">
                      <CardContent className="p-5">
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <Badge className={`${d.status === 'OPEN' ? 'bg-red-500' : d.status === 'IN_PROGRESS' ? 'bg-amber-500' : 'bg-green-500'} text-white border-0 text-xs`}>
                                {d.status === 'OPEN' ? 'Ouvert' : d.status === 'IN_PROGRESS' ? 'En cours' : 'Resolu'}
                              </Badge>
                            </div>
                          ...
                          {d.status !== 'RESOLVED' && (
                            <Button size="sm" className="bg-petblue hover:bg-petblue-dark text-white rounded-lg">Resoudre</Button>
                          )}
                        </div>
                      </CardContent>
                    </Card>
            ...
            {activeSection === 'stats' && (
              <div className="space-y-6">
                ...
                <div className="grid sm:grid-cols-2 gap-6">
                  <Card className="border-0 shadow-sm">
                    <CardHeader><CardTitle className="text-lg">Repartition par type</CardTitle></CardHeader>
                    <CardContent>
                      <div className="space-y-4">
                        <div>
                  ...
                  <Card className="border-0 shadow-sm">
                    <CardHeader><CardTitle className="text-lg">Repartition par role</CardTitle></CardHeader>
                    <CardContent>
                      <div className="space-y-4">
                        {[
                          ...
                          { label: 'Eleveurs', value: users.filter(u => u.role === 'BREEDER').length, color: 'bg-green-500' },
                    ...
                    <CardContent>
                      <div className="space-y-4">
                        {[
                          ...
                          { label: 'Reserves', value: reservedAnimals, color: 'bg-amber-500' },
                          { label: 'Adoptes', value: adoptedAnimals, color: 'bg-gray-400' },
                        ].map((s, i) => (
                          <div key={i}>
                  ...
                  <Card className="border-0 shadow-sm">
                    ...
                    <CardContent>
                      <div className="space-y-3">
                        <div className="flex items-center justify-between p-3 bg-muted/30 rounded-lg"><span className="text-sm">Total encaisse</span><span className="font-bold text-green-500">{totalRevenue.toLocaleString('fr-FR')} EUR</span></div>
                        <div className="flex items-center justify-between p-3 bg-muted/30 rounded-lg"><span className="text-sm">En attente</span><span className="font-bold text-amber-500">{pendingRevenue.toLocaleString('fr-FR')} EUR</span></div>
                        <div className="flex items-center justify-between p-3 bg-muted/30 rounded-lg"><span className="text-sm">Rembourse</span><span className="font-bold text-red-500">0 EUR</span></div>
                      </div>
                    </CardContent>
                  </Card>
                </div>
            ...
            {activeSection === 'settings' && (
              <div className="space-y-6">
                ...
                <Card className="border-0 shadow-sm">
                  <CardHeader><CardTitle className="text-lg">Parametres de la plateforme</CardTitle></CardHeader>
                  <CardContent className="space-y-4">
                    <div className="flex items-center justify-between p-4 bg-muted/30 rounded-lg">
                      <div><p className="font-medium">Commission sur les ventes</p><p className="text-sm text-muted-foreground">Pourcentage preleve sur chaque transaction</p></div>
                    ...
                    <div className="flex items-center justify-between p-4 bg-muted/30 rounded-lg">
                      ...
                      <Badge className="bg-red-500 text-white border-0">Desactive</Badge>
                    </div>
                    <div className="flex items-center justify-between p-4 bg-muted/30 rounded-lg">
                      <div><p className="font-medium">Mode maintenance</p><p className="text-sm text-muted-foreground">Desactiver temporairement l'acces au site</p></div>
                      <Badge className="bg-green-500 text-white border-0">Desactive</Badge>
                    </div>
                    <div className="flex items-center justify-between p-4 bg-muted/30 rounded-lg">
      ...
      <Dialog open={showAddAnimal} onOpenChange={setShowAddAnimal}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Remplissez les informations pour ajouter un nouvel animal a la plateforme.</DialogDescription>
          </DialogHeader>
          <div className="grid grid-cols-2 gap-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Select value={animalForm.sex} onValueChange={v => setAnimalForm({ ...animalForm, sex: v as 'MALE' | 'FEMALE' })}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent><SelectItem value="MALE">Male</SelectItem><SelectItem value="FEMALE">Femelle</SelectItem></SelectContent>
              </Select>
            </div>
            <div className="space-y-2">
              <Label>Age *</Label>
            ...
            <div className="space-y-2">
              ...
              <Select value={animalForm.ageUnit} onValueChange={v => setAnimalForm({ ...animalForm, ageUnit: v as 'MONTHS' | 'YEARS' })}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent><SelectItem value="MONTHS">Mois</SelectItem><SelectItem value="YEARS">Annees</SelectItem></SelectContent>
              </Select>
            </div>
            <div className="space-y-2">
            ...
            <div className="col-span-2 space-y-2">
              ...
              <Textarea value={animalForm.description} onChange={e => setAnimalForm({ ...animalForm, description: e.target.value })} placeholder="Decrivez l'animal..." rows={3} />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Select value={animalForm.status} onValueChange={v => setAnimalForm({ ...animalForm, status: v as 'AVAILABLE' | 'RESERVED' | 'ADOPTED' })}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent><SelectItem value="AVAILABLE">Disponible</SelectItem><SelectItem value="RESERVED">Reserve</SelectItem><SelectItem value="ADOPTED">Adopte</SelectItem></SelectContent>
              </Select>
            </div>
          </div>
          <DialogFooter>
            ...
            <Button className="bg-petblue hover:bg-petblue-dark text-white" onClick={() => {
              ...
              resetAnimalForm()
              showToast(`${animalForm.name} a ete ajoute`)
            }}>Ajouter</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={showEditAnimal} onOpenChange={setShowEditAnimal}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-4 py-4">
            ...
            <div className="space-y-2"><Label>Sexe</Label><Select value={animalForm.sex} onValueChange={v => setAnimalForm({ ...animalForm, sex: v as 'MALE' | 'FEMALE' })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="MALE">Male</SelectItem><SelectItem value="FEMALE">Femelle</SelectItem></SelectContent></Select></div>
            ...
            <div className="space-y-2"><Label>Prix (EUR)</Label><Input type="number" value={animalForm.price} onChange={e => setAnimalForm({ ...animalForm, price: Number(e.target.value) })} /></div>
            ...
            <div className="flex items-center gap-2"><input type="checkbox" checked={animalForm.isVaccinated} onChange={e => setAnimalForm({ ...animalForm, isVaccinated: e.target.checked })} className="rounded" /><Label>Vaccine</Label></div>
            <div className="flex items-center gap-2"><input type="checkbox" checked={animalForm.isApproved} onChange={e => setAnimalForm({ ...animalForm, isApproved: e.target.checked })} className="rounded" /><Label>Approuve</Label></div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowEditAnimal(false)}>Annuler</Button>
            <Button className="bg-petblue hover:bg-petblue-dark text-white" onClick={() => {
              if (selectedItem) { updateAnimal(selectedItem.id, animalForm); showToast(`${selectedItem.name} a ete modifie`) }
      ...
      <Dialog open={showDeleteAnimal} onOpenChange={setShowDeleteAnimal}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Etes-vous sur de vouloir supprimer <strong>{selectedItem?.name}</strong> ? Cette action est irreversible.</DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowDeleteAnimal(false)}>Annuler</Button>
            <Button variant="destructive" onClick={() => { if (selectedItem) { deleteAnimal(selectedItem.id); showToast(`${selectedItem.name} a ete supprime`) }; setShowDeleteAnimal(false) }}>Supprimer</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
          ...
          {selectedItem && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div className="flex items-center gap-2"><DollarSign className="h-4 w-4 text-muted-foreground" /> Prix : {selectedItem.price} EUR</div>
                <div className="flex items-center gap-2"><Calendar className="h-4 w-4 text-muted-foreground" /> Ajoute : {new Date(selectedItem.createdAt).toLocaleDateString('fr-FR')}</div>
                <div className="flex items-center gap-2">Vaccine : {selectedItem.isVaccinated ? 'Oui' : 'Non'}</div>
                <div className="flex items-center gap-2">Approuve : {selectedItem.isApproved ? 'Oui' : 'Non'}</div>
              </div>
      ...
      <Dialog open={showAddUser} onOpenChange={setShowAddUser}>
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2"><Label>Role *</Label><Select value={userForm.role} onValueChange={v => setUserForm({ ...userForm, role: v as any })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="ADOPTER">Adoptant</SelectItem><SelectItem value="SHELTER">Refuge</SelectItem><SelectItem value="BREEDER">Eleveur</SelectItem><SelectItem value="ADMIN">Admin</SelectItem></SelectContent></Select></div>
            <div className="space-y-2"><Label>Telephone</Label><Input value={userForm.phone} onChange={e => setUserForm({ ...userForm, phone: e.target.value })} placeholder="+33 6 00 00 00 00" /></div>
          ...
          <DialogFooter>
            ...
            <Button className="bg-petblue hover:bg-petblue-dark text-white" onClick={() => {
              ...
              resetUserForm()
              showToast(`${userForm.name} a ete cree`)
            }}>Creer</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={showEditUser} onOpenChange={setShowEditUser}>
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2"><Label>Role</Label><Select value={userForm.role} onValueChange={v => setUserForm({ ...userForm, role: v as any })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="ADOPTER">Adoptant</SelectItem><SelectItem value="SHELTER">Refuge</SelectItem><SelectItem value="BREEDER">Eleveur</SelectItem><SelectItem value="ADMIN">Admin</SelectItem></SelectContent></Select></div>
            <div className="space-y-2"><Label>Telephone</Label><Input value={userForm.phone} onChange={e => setUserForm({ ...userForm, phone: e.target.value })} /></div>
          ...
          <DialogFooter>
            ...
            <Button className="bg-petblue hover:bg-petblue-dark text-white" onClick={() => {
              if (selectedItem) { updateUser(selectedItem.id, userForm); showToast(`${selectedItem.name} a ete modifie`) }
      ...
      <Dialog open={showDeleteUser} onOpenChange={setShowDeleteUser}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={() => { if (selectedItem) { deleteUser(selectedItem.id); showToast(`${selectedItem.name} a ete supprime`) }; setShowDeleteUser(false) }}>Supprimer</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
          ...
          {selectedItem && (
            <div className="space-y-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <Badge variant="outline" className={selectedItem.role === 'ADMIN' ? 'border-purple-300 text-purple-600' : selectedItem.role === 'SHELTER' ? 'border-petorange text-petorange' : selectedItem.role === 'BREEDER' ? 'border-green-400 text-green-600' : 'border-petblue text-petblue'}>
                    {selectedItem.role === 'ADMIN' ? 'Admin' : selectedItem.role === 'SHELTER' ? 'Refuge' : selectedItem.role === 'BREEDER' ? 'Eleveur' : 'Adoptant'}
                  </Badge>
                </div>
              </div>
              <Separator />
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div><span className="text-muted-foreground">Telephone :</span><p className="font-medium">{selectedItem.phone || '-'}</p></div>
                ...
                <div><span className="text-muted-foreground">Verifie :</span><p className="font-medium">{selectedItem.isVerified ? 'Oui' : 'Non'}</p></div>
